...
let FavoriteEntityResolver = class FavoriteEntityResolver {
    ...
    async product(favorite, ctx) {
        ...
        if (!product) {
            const favoriteWithProduct = await this.connection.getRepository(ctx, favorite_entity_1.Favorite).findOne(favorite.id, {
                relations: ['product'],
            });
    ...
    async customer(favorite, ctx) {
        ...
        if (!customer) {
            const favoriteWithCustomer = await this.connection
                .getRepository(ctx, favorite_entity_1.Favorite)
                .findOne(favorite.id, {
                relations: ['customer'],
            });
